        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden h-full flex flex-col">
            <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                <h3 className="text-[11px] font-black text-slate-700 uppercase tracking-widest flex items-center gap-2">
                    ...
                    {t('vehicle_details.financial_overview')}
                </h3>
            </div>
            ...
            {vehicle?.do_not_pay && (
                <div className="mx-5 my-4 flex items-center gap-3 bg-orange-50 border-2 border-orange-400 text-orange-800 px-4 py-3 rounded-xl shadow-md">
                    ...
                    <div className="flex-1">
                        <p className="font-black text-[11px] uppercase tracking-widest">⚠ {t('vehicle_details.do_not_pay_warning_title')}</p>
                        <p className="text-[10px] mt-0.5 font-medium">{t('vehicle_details.do_not_pay_warning_text')}</p>
                    </div>
                </div>
            )}
            ...
            <div className="bg-slate-900 p-5 text-white">
                <div className="grid grid-cols-3 gap-4 text-center divide-x divide-slate-700">
                    <div>
                        <div className="text-[10px] text-slate-400 uppercase tracking-wider mb-1 font-bold">{t('vehicle_details.invoiced')}</div>
                        <div className="text-xl font-bold tracking-tight">{formatCurrency(totalInvoiced)}</div>
                    </div>
                    <div>
                        <div className="text-[10px] text-slate-400 uppercase tracking-wider mb-1 font-bold">{t('vehicle_details.paid')}</div>
                        <div className="text-xl font-bold text-green-400 tracking-tight">{formatCurrency(totalPaid)}</div>
                    </div>
                    <div>
                        <div className="text-[10px] text-slate-400 uppercase tracking-wider mb-1 font-bold">{t('vehicle_details.balance')}</div>
            ...
            <div className="p-5 flex-1 overflow-y-auto w-full">
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{t('vehicle_details.invoices')}</h4>
                    </div>
                    ...
                    <div className="space-y-3">
                        {(!invoices || invoices.length === 0) ? (
                            <div className="text-center py-8 border-2 border-dashed border-slate-100 rounded-xl bg-slate-50/50">
                                ...
                                <p className="text-sm text-slate-400 font-medium mb-4">{t('vehicle_details.no_invoices')}</p>
                            </div>
                        ) : (
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wide border ${inv.status === 'paid' ? 'bg-green-50 text-green-700 border-green-100' :
                                                    ...
                                                    {t(`status.${(inv.status || 'draft').toLowerCase()}`, { defaultValue: inv.status || 'Draft' })}
                                                </span>
                                            </div>
                                            <div className="text-xs text-slate-500 font-medium flex items-center gap-1">
                                                ...
                                                <span className="uppercase tracking-wider font-bold text-[10px] text-blue-600/80">
                                                    {inv.service_category || t('vehicle_details.invoice')}
                                                </span>
                                            </div>
                                        </div>
                                                ...
                                                if (bal > 0) {
                                                    return (
                                                        <div className="text-xs text-red-600 font-bold flex items-center justify-end gap-1 mt-0.5">
                                                            ...
                                                            {t('vehicle_details.due')}: {formatCurrency(bal)}
                                                        </div>
                                                    );
                                                }
                                                return null;
                                    ...
                                    {inv.quickbooks_invoice_id && (
                                        <div className="flex items-center gap-3 pt-2 border-t border-slate-100">
                                            <button
                                                ...
                                                title={t('vehicle_details.view')}
                                            >
                                                <Eye className="h-3 w-3" />
                                                {t('vehicle_details.view')}
                                            </button>
                                            <button
                                                ...
                                                title={t('vehicle_details.download')}
                                            >

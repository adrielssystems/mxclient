...
function ClientReadOnlyOwnershipDocuments({ vehicle }) {
    ...
    if (loading) {
        return <div className="text-xs text-slate-400 p-4 text-center">{t('vehicle_details.loading_documents')}</div>;
    }
    ...
    if (docs.length === 0) {
        return (
            <div className="border border-dashed border-slate-200 rounded-xl p-6 flex flex-col items-center justify-center text-center bg-slate-50">
                ...
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">{t('vehicle_details.no_documents_yet')}</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-2">
            <div className="flex justify-between items-center mb-1">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">{t('vehicle_details.ownership_documents')}</label>
            </div>
            {docs.map(doc => (
                <div key={doc.id} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg hover:border-blue-300 transition-colors group">
                    <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="min-w-0">
                            <p className="text-xs font-bold text-slate-700 truncate">{doc.doc_type || t('vehicle_details.document')}</p>
                    ...
                    <div className="flex items-center gap-1">
                        <a 
                            ...
                            title={t('vehicle_details.view_document')}
                        >
                            <Eye size={14} />
                        </a>
                        <a 
                            ...
                            className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors"
                            title={t('vehicle_details.download_document')}
                        >
                            <Download size={14} />
                        </a>
                    </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden h-full">
            ...
            <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
                <h3 className="text-[11px] font-black text-slate-700 uppercase tracking-widest flex items-center gap-2">
                    ...
                    {t('vehicle_details.vehicle_information')}
                </h3>
            </div>
            ...
            <div className="p-4 space-y-4">
                ...
                <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">{t('vehicle_details.description')}</label>
                    <div className="text-sm font-black text-slate-900 break-words leading-snug">
                        {vehicle.description ? vehicle.description.replace(/\s*\([^)]*\)\s*/g, '').trim() : t('vehicle_details.no_description')}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 block">{t('vehicle_details.purchase_date')}</label>
                        <div className="flex items-center gap-2 text-xs font-bold text-slate-700 bg-slate-50 px-3 py-2 rounded-lg border border-slate-100">
                            ...
                            {vehicle.purchase_date ? new Date(vehicle.purchase_date).toLocaleDateString(i18n.language === 'ar' ? 'ar-EG' : i18n.language || 'en-US') : '-'}
                        </div>
                    </div>
                    <div>
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 block">{t('vehicle_details.lot_number')}</label>
                ...
                {/* Origin & Location */}
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">{t('vehicle_details.origin_location')}</label>
                    <div className="bg-slate-50 border border-slate-100 rounded-lg p-3 space-y-3 text-xs">
                        <div className="flex flex-col gap-1">
                            <span className="font-bold text-slate-500 uppercase tracking-widest text-[9px]">{t('vehicle_details.auction')}</span>
                            <span className="font-bold text-slate-800">{vehicle.auction_name || '-'}</span>
                        </div>
                        <div className="flex flex-col gap-1">
                            <span className="font-bold text-slate-500 uppercase tracking-widest text-[9px]">{t('vehicle_details.location')}</span>
                ...
                {/* Owner / Client */}
                <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">{t('vehicle_details.owner_client')}</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">{t('vehicle_details.pin')}</label>
                    ...
                    <div className="space-y-1.5">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">{t('vehicle_details.buyer')}</label>
                ...
                {/* Dealer */}
                <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">{t('vehicle_details.dealer')}</label>
                    <div className="flex items-center gap-2 text-xs font-bold text-slate-700 bg-slate-50 px-3 py-2 rounded-lg border border-slate-100">
                        {vehicle.dl_number || '-'}
                    </div>
                </div>
